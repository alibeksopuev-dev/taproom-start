...
Что сделано — Задание 3: Server Function для заказа
Создан src/actions/order.ts
Это новый файл — серверная функция submitOrder. Весь код внутри .handler() выполняется только на сервере, никогда не попадает в браузер.

Что делает функция:

Принимает данные заказа (позиции, стол, способ оплаты)
Валидирует их через zod-схему — если что-то не то, бросает ошибку до Supabase
Создаёт запись в orders
Создаёт все order_items
Возвращает полный заказ с позициями
Ключевое отличие от старого кода:

Старый код (useCreateOrder) Новый (submitOrder)
Supabase ключ Виден в браузере в Network tab Только на сервере
Запросы к БД Из браузера напрямую Через сервер
Валидация Нет (TypeScript только) Zod на сервере
Обновлён src/queries/orders.ts
useCreateOrder теперь вызывает submitOrder({ data: request }) вместо прямых запросов к Supabase. CartSummary менять не нужно — он использует useCreateOrder как раньше, просто внутри теперь другой транспорт.

В данной версии TanStack Start метод называется .inputValidator(), а не .validator() как в документации — это была бы ошибка, нашёл правильный API из типов пакета.
